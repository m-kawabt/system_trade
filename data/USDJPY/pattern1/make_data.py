...
dest_input_file_path = '/raidc/m-kawabt/system_trade/data/USDJPY/pattern1/input.csv'
if os.path.exists(dest_gt_file_path):
    os.remove(dest_gt_file_path)
if os.path.exists(dest_input_file_path):
    os.remove(dest_input_file_path)
gt_list = []
            ...
            if cluster:
                ...
                if time_lapse > td(minutes=1):
                    if time_lapse == td(minutes=2):
                        ...
                        with open(dest_input_file_path, 'a') as f:
                            writer = csv.writer(f)
                            writer.writerows(cluster)
                        cluster = []
            cluster.append(list([t, oPen, high, low, close]))
        with open(dest_gt_file_path, 'w') as f:
            writer = csv.writer(f)
